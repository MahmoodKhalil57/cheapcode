/**
 * Re-score the captured runs against the corrected benchmark gold.
 * Reads results.jsonl (captured during the live run with the original
 * — incorrect — gold) and re-applies scoring with the corrected
 * benchmark.ts. Writes 3-axis-comparison-corrected.json.
 *
 * Background: t05-counting's gold was set to "8" but the true vowel
 * count of "cheapcode is awesome" is 9. Both baseline and wrapper
 * answered 9 — i.e. both got it right. The original run's 90% score
 * was a benchmark-author error, not a model error.
 */
...
const lines = readFileSync("runs/experiment-1-attempt-1/results.jsonl", "utf-8")
  .trim()
  .split("\n")
  .filter(Boolean)
...
const summary = {
  attempt: "experiment-1-attempt-1-rescored",
  date: new Date().toISOString(),
  N: TASKS.length,
  benchmark_correction: "t05-counting gold corrected from 8 to 9 (post-experiment author audit)",
...
writeFileSync("runs/experiment-1-attempt-1/3-axis-comparison-rescored.json", JSON.stringify(summary, null, 2))
...
console.log(`Completion: ${(wrapperCompletion * 100).toFixed(0)}% / ${(baselineCompletion * 100).toFixed(0)}% = ${completionRatio.toFixed(3)}× (target ≥${COMPLETION_TARGET}) ${completionPass ? "✓" : "✗"}`)
