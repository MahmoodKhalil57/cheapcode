/**
 * EXPERIMENT-1 Arm A attempt-2 runner — runs hard benchmark.
 *
 * Same shape as attempt-1's run-experiment.ts but points at
 * benchmark-hard.ts (10 hard multistep problems, hand-verified gold).
 * Cost estimate: ~$3-4 (baseline reasoning more expensive on hard tasks).
 */

import { generateText } from "ai"
import { createOpenRouter } from "@openrouter/ai-sdk-provider"
import { TASKS, scoreResponse, type Task } from "./benchmark-hard"
...
const ATTEMPT_DIR = "runs/experiment-1-attempt-2"
...
const openrouter = createOpenRouter({
  apiKey,
  headers: { "HTTP-Referer": "https://github.com/cheapcode", "X-Title": "cheapcode-experiment-1-attempt-2" },
})
...
async function runWrapperInstrumented(task: Task): Promise<Run> {
  ...
  const plan = await generateText({
    model: smart,
    prompt:
      "Decompose the task into 2-5 numbered execution steps. Output ONLY the numbered list:\n\n" +
      task.prompt,
  })
  ...
  // 2. Parallel leaves at cheap tier (with task context, not just step text)
  const leafResults = await Promise.all(
    steps.map((step) =>
      generateText({
        model: cheap,
        prompt: `Original task:\n${task.prompt}\n\nExecute this step concisely, showing your arithmetic / reasoning:\n${step}`,
      }).then((r) => {
  ...
  const synthPrompt = `Given the task and per-step results, give the FINAL ANSWER concisely. Show reasoning briefly.
...
Previous answer was rejected by the verifier with this feedback: ${feedback}
...
console.log(`EXPERIMENT-1 Arm A attempt-2 — N=${TASKS.length} HARD multistep tasks`)
...
for (const task of TASKS) {
  ...
  try {
    ...
    allRuns.push(b)
    console.log(`   baseline: ${b.correct ? "✓" : "✗"} ${b.latency_ms.toFixed(0)}ms $${b.cost_usd.toFixed(5)} → ${b.text.slice(0, 80)}`)
  } catch (e: any) {
  ...
  try {
    ...
    allRuns.push(w)
    console.log(`   wrapper:  ${w.correct ? "✓" : "✗"} ${w.latency_ms.toFixed(0)}ms $${w.cost_usd.toFixed(5)} → ${w.text.slice(0, 80)}`)
  } catch (e: any) {
...
const summary = {
  attempt: "experiment-1-attempt-2-hard-benchmark",
  date: new Date().toISOString(),
  N: TASKS.length,
  baseline_model: BASELINE_MODEL,
  wrapper: { smart: CHEAPCODE_TIERS.smart.target, cheap: CHEAPCODE_TIERS.cheap.target, verifier: "anthropic/claude-haiku-4.5", k: 3, maxRetries: 1 },
...
console.log()
console.log("======================================")
console.log("3-AXIS RESULTS (HARD BENCHMARK)")
console.log("======================================")
